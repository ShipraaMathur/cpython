...
small_stmt_rule(Parser *p)
{
    ...
    { // ('pass' | 'proceed')
        ...
        D(fprintf(stderr, "%*c> small_stmt[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "('pass' | 'proceed')"));
        void *_tmp_15_var;
        if (
            (_tmp_15_var = _tmp_15_rule(p))  // 'pass' | 'proceed'
        )
        {
            D(fprintf(stderr, "%*c+ small_stmt[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "('pass' | 'proceed')"));
            Token *_token = _PyPegen_get_last_nonnwhitespace_token(p);
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s small_stmt[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "('pass' | 'proceed')"));
    }
    { // &'del' del_stmt
        if (p->error_indicator) {
            D(p->level--);
...
except_block_rule(Parser *p)
{
    ...
    { // 'except' expression ['as' NAME] ':' block
        ...
        void *t;
        if (
            (_keyword = _PyPegen_expect_token(p, 519))  // token='except'
            &&
            (e = expression_rule(p))  // expression
            &&
...
finally_block_rule(Parser *p)
{
    ...
    { // 'finally' ':' block
        ...
        asdl_seq* a;
        if (
            (_keyword = _PyPegen_expect_token(p, 520))  // token='finally'
            &&
            (_literal = _PyPegen_expect_token(p, 11))  // token=':'
            &&
...
function_def_raw_rule(Parser *p)
{
    ...
    { // 'def' NAME '(' params? ')' ['->' expression] ':' func_type_comment? block
        ...
        void *tc;
        if (
            (_keyword = _PyPegen_expect_token(p, 521))  // token='def'
            &&
...
class_def_raw_rule(Parser *p)
{
    ...
    { // 'class' NAME ['(' arguments? ')'] ':' block
        ...
        asdl_seq* c;
        if (
            (_keyword = _PyPegen_expect_token(p, 522))  // token='class'
            &&
            (a = _PyPegen_name_token(p))  // NAME
            &&
...
lambdef_rule(Parser *p)
{
    ...
    { // 'lambda' lambda_parameters? ':' expression
        ...
        expr_ty b;
        if (
            (_keyword = _PyPegen_expect_token(p, 523))  // token='lambda'
            &&
...
inversion_rule(Parser *p)
{
    ...
    { // 'not' inversion
        ...
        expr_ty a;
        if (
            (_keyword = _PyPegen_expect_token(p, 524))  // token='not'
            &&
            (a = inversion_rule(p))  // inversion
        )
        {
...
isnot_bitwise_or_rule(Parser *p)
{
    ...
    { // 'is' 'not' bitwise_or
        ...
        expr_ty a;
        if (
            (_keyword = _PyPegen_expect_token(p, 525))  // token='is'
            &&
            (_keyword_1 = _PyPegen_expect_token(p, 524))  // token='not'
            &&
            (a = bitwise_or_rule(p))  // bitwise_or
        )
        {
...
atom_rule(Parser *p)
{
    ...
    { // 'None'
        ...
        Token * _keyword;
        if (
            (_keyword = _PyPegen_expect_token(p, 528))  // token='None'
        )
        {
            D(fprintf(stderr, "%*c+ atom[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'None'"));
            Token *_token = _PyPegen_get_last_nonnwhitespace_token(p);
    ...
    { // '__new_parser__'
        ...
        Token * _keyword;
        if (
            (_keyword = _PyPegen_expect_token(p, 529))  // token='__new_parser__'
        )
        {
            D(fprintf(stderr, "%*c+ atom[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'__new_parser__'"));
...
_tmp_15_rule(Parser *p)
{
    ...
    { // 'pass'
        ...
        Token * _keyword;
        if (
            (_keyword = _PyPegen_expect_token(p, 530))  // token='pass'
        )
        {
    ...
    { // 'proceed'
        ...
        Token * _keyword;
        if (
            (_keyword = _PyPegen_expect_token(p, 531))  // token='proceed'
        )
        {
            D(fprintf(stderr, "%*c+ _tmp_15[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'proceed'"));
            _res = _keyword;
...
_tmp_35_rule(Parser *p)
{
    ...
    { // 'as' NAME
        ...
        expr_ty z;
        if (
            (_keyword = _PyPegen_expect_token(p, 532))  // token='as'
            &&
